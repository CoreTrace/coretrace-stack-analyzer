...
- Each SARIF rule carries the CWE tags of its diagnostics. A rule that detects a security weakness
  also carries the `security` tag and a `security-severity` score, so code scanning lists its
  alerts as security alerts, ranked critical to low. The scores are in `src/report/ReportSerialization.cpp`.
