...
static bool buildParamTypeInfo(const llvm::DIType *type,
                               ParamTypeInfo &info)
{
    using namespace llvm;
    if (!type)
...
static std::string buildTypeString(const ParamTypeInfo &info,
                                   const std::string &baseName,
                                   bool addPointeeConst,
                                   bool includePointerConst,
...
static std::string buildPointeeQualPrefix(const ParamTypeInfo &info,
                                          bool addConst)
{
    std::string out;
    if (addConst)
...
static ParamDebugInfo getParamDebugInfo(const llvm::Function &F,
...
static bool calleeParamIsReadOnly(const llvm::Function *callee,
                                  unsigned argIndex)
{
...
static bool callArgMayWriteThrough(const llvm::CallBase &CB,
                                   unsigned argIndex)
{
    using namespace llvm;
...
static bool valueMayBeWrittenThrough(const llvm::Value *root,
                                     const llvm::Function &F)
{
    using namespace llvm;
    (void)F;
                                         ...
                                         std::vector<ConstParamIssue> &out)
{
    ...
    for (Argument &Arg : F.args()) {
        ...
        issue.pointerConstOnly = typeInfo.isPointer && typeInfo.pointerConst && !typeInfo.pointeeConst;
        ...
        if (typeInfo.isRvalueReference) {
            ...
            issue.suggestedType = buildTypeString(typeInfo, baseName, true, false, issue.paramName);
        }
    ...
    for (const auto &cp : constParamIssues)
    {
        ...
        if (cp.isRvalueRef) {
            ...
            body << "  " << prefix << "ConstParameterNotModified." << subLabel
                 ...
                 << (cp.isReference ? "referred" : "pointed")
                 << " object\n";
        }
                   ...
                   const std::string &inputFile)
{
    ...
    for (std::size_t i = 0; i < result.diagnostics.size(); ++i) {
        ...
        const std::string ruleId = d.ruleId.empty()
            ? std::string(ctrace::stack::enumToString(d.errCode))
            : d.ruleId;
